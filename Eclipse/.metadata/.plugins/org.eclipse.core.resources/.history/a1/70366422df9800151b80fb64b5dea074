...
public class SAVOP {
	...
	public static boolean actualizaInfoDeputado(String idDeputado,
			String[][] deputados, int numDeputados) {
		int pos, nc, np, ntp;
	...
	public static void resultadosVotacao(char votacoes[], String deputados[][],
			int numDeputados) {
	...
	private static void listagemcharPaginada(char v[], String deputados[][]) {
		for (int i = 0; i < v.length; i++) {
			System.out.printf("%-6s | %-30s | %-10s|%-12s | %s \n",
					deputados[i][0], deputados[i][1], deputados[i][2],
					deputados[i][3], v[i]);
		}
	}
	...
	public static char[] lerVotacoes(int numDeputados, String deputados[][])
			throws FileNotFoundException {
		char[] votacoes = new char[numDeputados];
		System.out.println("Introduzir nome da votação desejada:");
		String assuntoVotado = sc.next();
		...
		while (fileIn.hasNext() && inx < numDeputados) {
			...
			if (linha.length() > 0) {
				linha = linha.trim();
				indiceDeputado = pesquisarDeputadoPorID(linha.substring(0, 5),
						numDeputados, deputados);
